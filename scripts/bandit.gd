...
## Разбойник — патруль + агро на игрока.
...
func _build() -> void:
	...
	_sprite.texture = PixelArt.bandit_tex()
...
func setup_patrol(center_x: float, width: float) -> void:
...
func _physics_process(delta: float) -> void:
	...
	if is_instance_valid(_player):
		...
		if absf(dx) < 220.0 and dy < 80.0:
			...
			# jump if player higher and close
