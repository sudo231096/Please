...
# --- профиль игрока (главное меню в стиле Rust Mobile) ---
var player_id := 0            # уникальный ID аккаунта
var level := 1
var xp := 0                   # опыт внутри текущего уровня
var coins := 500              # основная валюта (золото)
var rp := 25                  # премиум-валюта (кристаллы)
var scrap_currency := 0       # металлолом-валюта меню
var has_battlepass := false
var bp_tier := 1
var free_supply_at := 0       # unix-время следующей бесплатной доставки
var free_coins_at := 0        # unix-время следующих бесплатных монет
var run_active := false       # есть ли незавершённый матч (для кнопки ПРОДОЛЖИТЬ)


func xp_needed() -> int:
	# требуемый опыт растёт с уровнем
...
func supply_ready() -> bool:
	return now_ts() >= free_supply_at


func free_coins_ready() -> bool:
	return now_ts() >= free_coins_at


func claim_supply() -> Dictionary:
	# бесплатная доставка: набор ресурсов раз в 30 минут
	if not supply_ready():
		return {}
	free_supply_at = now_ts() + 1800
	var loot := {"wood": 200, "stone": 150, "cloth": 40, "scrap": 15}
	for k in loot:
		add_item(String(k), int(loot[k]))
	save_profile()
	return loot


func claim_free_coins() -> int:
	# бесплатные монеты раз в 15 минут
	if not free_coins_ready():
		return 0
	free_coins_at = now_ts() + 900
	var amount := 75 + level * 5
	coins += amount
	save_profile()
	return amount


func time_left(ts: int) -> String:
	var left: int = maxi(0, ts - now_ts())
...
func save_profile() -> void:
	...
	cfg.load(SAVE_PATH)
	cfg.set_value("prof", "id", player_id)
	cfg.set_value("prof", "name", player_name)
	cfg.set_value("prof", "level", level)
	cfg.set_value("prof", "xp", xp)
	cfg.set_value("prof", "coins", coins)
	cfg.set_value("prof", "rp", rp)
	cfg.set_value("prof", "scrapc", scrap_currency)
	cfg.set_value("prof", "bp", has_battlepass)
	cfg.set_value("prof", "bp_tier", bp_tier)
	cfg.set_value("prof", "supply_at", free_supply_at)
	cfg.set_value("prof", "coins_at", free_coins_at)
	cfg.save(SAVE_PATH)
...
func load_profile() -> void:
	...
	if cfg.load(SAVE_PATH) != OK:
		return
	player_id = int(cfg.get_value("prof", "id", 0))
	player_name = String(cfg.get_value("prof", "name", player_name))
	level = int(cfg.get_value("prof", "level", 1))
	xp = int(cfg.get_value("prof", "xp", 0))
	coins = int(cfg.get_value("prof", "coins", 500))
	rp = int(cfg.get_value("prof", "rp", 25))
	scrap_currency = int(cfg.get_value("prof", "scrapc", 0))
	has_battlepass = bool(cfg.get_value("prof", "bp", false))
	bp_tier = int(cfg.get_value("prof", "bp_tier", 1))
	free_supply_at = int(cfg.get_value("prof", "supply_at", 0))
